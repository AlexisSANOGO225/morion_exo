export default class GameBoard extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (name == "pseudo") this.pseudo = newValue;
    if (name == "time") this.time = newValue;
    if (name == "first-player") this.firstPlayer = newValue;
  }
  ...
  initGame() {
    ...
    this.render();
    this.styleCurrentPlayer();
  }

  styleCurrentPlayer() {
    let stylePlayer = this.shadowRoot.getElementById("player-1-pseudo");
    let styleOrdi = this.shadowRoot.getElementById("player-2-pseudo");
    if (this.getOtherPlayer() == "X") {
      stylePlayer.style.color = "#fff";
      styleOrdi.style.color = "green";
    } else if (this.getOtherPlayer() == "O") {
      styleOrdi.style.color = "#fff";
      stylePlayer.style.color = "green";
    }
  }

  dataPlayer(){
    let key = this.players['X'].name;
    for (let i = 0; i < localStorage.length; i++){
      let index = localStorage.key(i);
      let value = JSON.parse(localStorage.getItem(index));
      if(key == index){
        this.gamePart += value.gamePart;
        this.victories += value.victories;
        this.defeats += value.defeats;
        this.equality += value.equality;
        
      }
    }
    let statePlayer = {
      name: this.players['X'].name,
      gamePart: this.gamePart,
      victories: this.victories,
      defeats: this.defeats,
      equality: this.equality
    }
    localStorage.setItem(key, JSON.stringify(statePlayer));
  }
  ...
  editTimeForplayers() {
    this.shadowRoot.getElementById(
      "player-1-time"
    ).textContent = `00:${this.players["X"].timeRemaining}`;
    /**
     * 00:29
     */
    this.shadowRoot.getElementById(
      "player-2-time"
    ).textContent = `00:${this.players["O"].timeRemaining}`;
    /**
     * 00:29
     */
  }
  ...
  computerPlay() {
    ...
    if (emptyCellNumbers.length > 0) {
      const randomNumber =
        emptyCellNumbers[Math.floor(Math.random() * emptyCellNumbers.length)];
